...
class Histogram(Renderable, RootObject):
    ...
    def rebin(self, parent, key, n):
        ...
        def rebin(h):
            ...
            new_hist.Rebin(n)
            new_hist.GetXaxis(
            ).SetRange(self.obj.GetXaxis().GetFirst(), self.obj.GetXaxis().GetLast())
            new_hist.GetYaxis(
            ).SetRange(self.obj.GetYaxis().GetFirst(), self.obj.GetYaxis().GetLast())
            new_hist.GetZaxis(
            ).SetRange(self.obj.GetZaxis().GetFirst(), self.obj.GetZaxis().GetLast())
            return new_hist
    ...
    class ExplodeSlotKey(object):
        ...
        @property
        def tup(self):
            return self.axis, self.bin, self.pretty_name

        def __eq__(self, rhs):
            return self.tup == rhs.tup

        def __lt__(self, rhs):
            return self.pretty_name < rhs.pretty_name
