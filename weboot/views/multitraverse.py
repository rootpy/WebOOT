...
def build_plot_view(request, values):
    ...
    for value in values:
        ...
        if not value[-1]:
            ...
            continue
        content.append('<img class="plot" title="{0!r}" src="{1.icon_url}" />'.format(
            value[:-1], value[-1], request.query_string))
    return content
...
def view_multitraverse(multitravese_context, request):
    ...
    for index_tuple, context in sorted(multitravese_context.indexed_contexts):
        ...
        if prev_idx != this_idx:
            content.append('<h2><a href="{1.url}">{0}</a></h2>'.format(
            ...
            if isinstance(context, MultipleTraverser):
                for i, c in context.indexed_contexts:
                    content.append('<a href="{1.url}?{2}">{0}</a>'.format(
                        " / ".join(str_index_tuple), c, request.environ.get("QUERY_STRING", "")))
            else:
...
def view_multitraverse_render(context, request):
    ...
    with render_canvas() as c:
        if "logx" in request.params:
            c.SetLogx()
        if "logy" in request.params:
            c.SetLogy()
        if "logz" in request.params:
            c.SetLogz()
