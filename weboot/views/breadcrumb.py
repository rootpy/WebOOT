...
def build_breadcrumbs(context):
    ...
    def build_submenu(c, name, fragments):
        ...
        a('<li><a href="{0.url}*/{1}/?{2}">*</a> <a href="{0.url}*/!compose/stack/{1}/?{2}">Compose</a></li>'.format(c, final_url_part, context.request.environ.get("QUERY_STRING", "")))
        for x in good_keys:
            #a('<li><a href="{0.url}">{0.__name__}</a> <!--<style type="text/css">color:white;</style>({1})--></li>'.format(c[x], "/".join(n)))
            a('<li><a href="{0.url}{1}/{2}/?{3}">{1}</a></li>'.format(c, x, final_url_part, context.request.environ.get("QUERY_STRING", "")))
        
        return "<ul>{0}</ul>".format("".join(content)) if content else ""
    
    a('<ul id="menu">')
    for this_context, fragments in zip(complete_lineage, remaining_fragments):
        args = this_context, build_submenu(this_context.__parent__, this_context.__name__, fragments[1:])
